...
1. **Activating a piece** — **100,000 \$AGNTS, the same for every tier.** Half is burned, half goes
   to treasury. Plus 10% of that amount in ETH.
...
**5,950 pieces are live, and about half of those sit in the vault.** So roughly **2,975** are in
public hands and could actually be switched on.

| | Pieces | At 100,000 each | Share of the 500M circulating |
|---|---|---|---|
| In public hands | ~2,975 | ~297,500,000 | ~60% |
| Every live piece | 5,950 | 595,000,000 | ~119% |

Activating everything held publicly would take **about 60% of the circulating supply**, and half of
every activation is **burned** — so activating destroys the means to activate. Demand is real, but
it is capped by a shrinking piece count, not open-ended.

This is why the cost is **a number that can be lowered and never raised**. As the token moves, the
amount comes down. See [\$AGNTS &amp; the Vault](/agnts/token).
...
Being straight about it: these are real and repeating, but they are bounded by how often a
shrinking collection actually trades and climbs — **5,950 pieces today, down from 6,969 minted.**
This is not an uncapped source of demand and should not be read as one.
