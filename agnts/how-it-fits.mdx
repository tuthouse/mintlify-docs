...
contract. The first launch is free, the next costs a dollar, and **the creator keeps the majority
share of the trading fee on their own token**, claimable at any time.
...
Token holders are not in that split. The 1.095% taken on \$AGNTS trades goes **100% to the
protocol** and none of it enters the pool. Holding the token pays you nothing.
...
1. **Activating a piece** — **100,000 \$AGNTS, the same for every tier.** Half is burned, half goes
   to treasury. Plus **10% of the piece's floor** in ETH, which does scale with tier.
2. **Buying a piece out of the vault** — the swap amount for that tier, plus the vault fee.

The token is what you spend to use the collection.
...
Every sale and every climb puts a piece back on the table. The collection is **5,950 today, down
from 6,969 minted**, and it only gets smaller.
