...
Trades and bridges route through **[Relay](https://relay.link)**, so your agent can move assets across **every chain Relay supports** — all the major EVM chains (Base, Ethereum, Robinhood Chain, Arbitrum, Optimism, Polygon, and many more) plus **Solana**. You're not limited to a handful of networks.
...
**NFT fees.** When your agent mints or buys an NFT for you, AGNT takes **5% on mints** and **1% on buys** — a percentage of the price, on top of which you pay network gas. **Selling** an NFT carries no AGNT fee (only the marketplace's own fees and any creator royalties apply).
